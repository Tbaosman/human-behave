...
def run_debate_simulation(
    ...
    # Sample patients
    if shared_sample_path:
        sample_path = Path(shared_sample_path)
        if not sample_path.exists():
            sample_path = OUTPUTS_DIR / shared_sample_path
        sample_df = pd.read_parquet(sample_path)
        print(f"Using shared sample: {len(sample_df)} patients from {sample_path}")
    elif balanced:
